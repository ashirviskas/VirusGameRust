...
const BRICK_SIZE: Vec2 = const_vec2!([15., 15.]);
// These values are exact
const GAP_BETWEEN_PADDLE_AND_BRICKS: f32 = 0.0;
...
impl BrickColor {
    fn to_color(&self) -> Color {
        match self {
            BrickColor::Red => Color::rgb(1.0, 0.0, 0.0),
            BrickColor::Green => Color::rgb(0.0, 1.0, 0.0),
            BrickColor::Blue => Color::rgb(0.0, 0.0, 1.0),
        }
    }
}
...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    for row in 0..n_rows {
        for column in 0..n_columns {
            let brick_type = should_spawn_type(row, column, n_rows);
            let brick_color = match brick_type {
                0 => BrickColor::Red,
                1 => BrickColor::Green,
                _ => BrickColor::Blue,
